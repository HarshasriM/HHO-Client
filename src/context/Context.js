...
export const AppProvider = ({ children }) => {
  ...
  useEffect(() => {
    const getMoney = async()=>{
        try {
          const response = await axios.get(`${apiUrl}/api/money/get-money`);
          const data = response.data;
          // console.log(data.moneyResp[0].amount);
          // console.log("amount: ",data.money[0].amount);
          // localStorage.setItem('totalBalance', data.money[0].amount);
          setTotalBalance(data.moneyResp[0].amount);
          setDonatedAmt(data.moneyResp[0].donated_amount)
